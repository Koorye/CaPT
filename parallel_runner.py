...
class ParallelRunner(object):
    ...
    def run(self):
        ...
        try:
            ...
            for root, _, filenames in os.walk(output_cfg['root']):
                for filename in filenames:
                    if filename == 'exceptions.txt':
                        exception_path = osp.join(root, filename)
                        break
    ...
    def run_single(self, opts=[]):
        ...
        if cfg['mode'] == 'b2n':
            ...
            commands = self.get_base_commands(opts)
        elif cfg['mode'] == 'contiual':
            commands = self.get_contiual_commands(opts)
        else:
    ...
    def get_base_commands(self, opts):
        data_cfg = self.data_cfg
        train_cfg = self.train_cfg
        output_cfg = self.output_cfg
    ...
    def get_contiual_commands(self, opts):
        data_cfg = self.data_cfg
        train_cfg = self.train_cfg
        output_cfg = self.output_cfg
        ...
        # training base on all datasets
        for dataset in datasets:
            for seed in seeds:
                cmd = get_command(data_root, seed, trainer, dataset, cfg, root, 
                                  shots, dataset, loadep, opts, mode='contiual', train=True, base=True)
                commands.append(cmd)

        # training new on all datasets
        for dataset in datasets:
            for seed in seeds:
                cmd = get_command(data_root, seed, trainer, dataset, cfg, root, 
                                  shots, dataset, loadep, opts, mode='contiual', train=True, base=False)
                commands.append(cmd)

        # testing new on base on all datasets
        for dataset in datasets:
            for seed in seeds:
                cmd = get_command(data_root, seed, trainer, dataset, cfg, root, 
                                  shots, dataset, loadep, opts, mode='contiual', train=False)
                commands.append(cmd)
